...
def unit_test(
    # Unit test
    _epoch=25,            # 25  int, the number of epochs
    _batch_size=32,       # 16  int, the batch size
    _lr=1e-3,             # 1e-3  the learning rate
    _weight_decay=1e-5,   # 1e-5  float, the weight decay
    _factor=0.2,          # 0.5  float, the factor
    _patience=10,         # 5  int, the patience
    _verbose=True,
    _x_offset=-28077.21,  # -28077.21  int, the x offset
    _y_offset=6207.34,    # 6207.34  int, the y offset
    _path_tra="./train_DNN.csv",   # str, the path to the training data
    _path_tes="./test_DNN.csv",    # str, the path to the test data
    _fig_title="DNNloss.pdf",      # str, the title of the figure
    _res_title="DNNpre_res.csv",   # str, the title of the result
):
    ...
    plt.xticks(np.arange(0, _epoch * 2 + 1, step=5))  # 让 x 轴刻度正确对齐
    plt.ylim(0, 1)  # 设定合理的 y 轴范围
    plt.yticks(np.arange(0, 1.05, 0.05))  # 设定 y 轴刻度
