...
# -------------------------------
# Main function: SAE is trained and DNN is trained in the stage
# -------------------------------
def unit_test(
    sae_epoch=15,        # Number of epochs for SAE training in each stage
    dnn_epoch=15,        # Number of epochs for DNN training in each stage
    _batch_size=32,
    _lr=1e-3,
    _weight_decay=1e-5,
    _factor=0.2,
    ...
    # -------------------------------
    # Initialize the model and optimizer globally (key change)
    # -------------------------------
    # SAE part: Encoder + Decoder
    ...
    # -------------------------------
    # Stage configuration: Datasets and corresponding loss weights used in each stage
    # -------------------------------
    stage_configs = [
        [(tra_loader1, 1.0)],                                        # Stage 1: Training set 1 only
        [(tra_loader1, 0.5), (tra_loader2, 1.0)],                     # Stage 2: Training set 1 (0.5) and training set 2 (1)
        [(tra_loader1, 0.25), (tra_loader2, 0.5), (tra_loader3, 1.0)]   # Stage 3: Training set 1 (0.25), training set 2 (0.5), training set 3 (1)
    ]

    total_loss = []  # Used to record the loss of DNN training in all phases
    ...
    for stage_id, loaders_weights in enumerate(stage_configs, start=1):
        ...
        # -------------------------------
        # SAE training part (key modification: use of global model and optimizer)
        # -------------------------------
        sae.train()
        ...
        # -------------------------------
        # DNN training part (key modification: use global model and optimizer)
        # -------------------------------
        model.train()
        with open(os.path.join(res_path, "DNN.log"), "a") as f:
            f.write(f"\nStage {stage_id} DNN Training\n")

        stage_dnn_loss = []  # Only the DNN loss is recorded
