...
#         #     self.save_checkpoint(loss_in, model)
#         #     self.counter = 0
#         if self.best_score is None or loss_in < self.best_score - self.delta:
#             self.best_score = loss_in  # 更新最佳 loss
#             self.save_checkpoint(loss_in, model)  # 保存模型
#             self.counter = 0  # 计数归零
#         else:
#             self.counter += 1
#             if self.counter >= self.patience:
...
def unit_test(
    ...
    for i in range(int(epoch * 2)):
        ...
        with torch.no_grad():
            ...
            # # 计算验证集的损失
            # val_loss = mae  # 可以根据需要选择合适的损失函数
            # early_stopping(val_loss, model)  # 调用早停机制
    ...
    # 保存训练好的模型
    # torch.save(sae.state_dict(), os.path.join(res_path, "trained_SAE.pth"))
